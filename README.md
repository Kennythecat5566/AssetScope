...
- 持股日 K 線與收盤趨勢圖，支援持有均價線與期間統計
- 持股可切換日線、月線、季線與年線資料範圍
- 總覽、持股、交易、同步分類頁可點選或左右滑動
- 總資產日期趨勢圖，支援雙指縮放與拖曳檢視
- 全域 TWD／USD 顯示切換與自動更新 USD/TWD 匯率
- 圖表顯示日期、價格／金額及幣別座標單位
- 三個隔離實盤的 AI 模擬交易機器人，分別自由交易美股、台股與全部個股
- 機器人詳細頁可查看完整虛擬交易紀錄與資產績效趨勢
- 機器人績效可同圖比較台灣加權指數與 S&P 500
- 機器人摘要與完整交易紀錄分開按需載入，降低同步等待
- 交易與消費長列表採用惰性載入，分頁列支援水平捲動
- 可透過 Gmail read-only OAuth 匯入信用卡消費通知到消費分析
- App 啟動時自動檢查 GitHub Release，也可從右上角手動檢查、下載並安裝更新
- 台幣與美元持倉換算
- 機構資產配置
- 成本、淨值、未實現損益與報酬率
- 已實現損益、股息收入與總投資報酬
- Firstrade 買入／賣出／股息交易時間軸
- 永豐證券持倉買進批次與 Shioaji 可查期間的已實現賣出明細
- 交易與績效會保存於手機，重開 App 後仍可查看
- CSV 匯入及本機持久化
